...
PlayerCharacter * ArcherCharacterFactory::create_player(std::string &name) {
    ...
    auto *pArcher = new Archer(name, abilities);
    return new PlayerCharacter(pArcher, 1, 0);
}

BaseCharacter *ArcherCharacterFactory::create_random_opponent(unsigned int level) {
    ...
    auto *archer = new Archer(random_choice(NAMES), abilities);
    return archer;
}